...
def evaluate(data, model, name):
    ...
    # print("gggggggggg",(np.array(gold_results)).shape,gold_results)
    # print("pppppppppp",(np.array(pred_results)).shape,pred_results)
    acc, p, r, f = get_ner_fmeasure(gold_results, pred_results, data.tagScheme)
    report = classification_report(y_true=gold_results, y_pred=pred_results, digits=4)
    return speed, acc, p, r, f, pred_results, gazes, report
...
def train(data, save_model_dir, seg=True):
    ...
    for idx in range(data.HP_iteration):
        ...
        # print("ffffffffffff",data.train_Ids)
        total_batch = train_num // batch_size + 1
        for batch_id in range(total_batch):
            start = batch_id * batch_size
            end = (batch_id + 1) * batch_size
            ...
            loss, tag_seq = model.neg_log_likelihood_loss(gaz_list, batch_word, batch_biword, batch_wordlen, layer_gaz,
                                                          gaz_count, gaz_chars, gaz_mask, gazchar_mask, mask.bool(),
        ...
        speed, acc, p, r, f, pred_labels, gazs, _ = evaluate(data, model, "dev")
        ...
        if current_score > best_dev:
            if seg:
                logger.info("Exceed previous best f score: %d" % (best_dev))

            else:
                logger.info("Exceed previous best acc score: %d" % (best_dev))
        ...
        # ## decode test
        speed, acc, p, r, f, pred_labels, gazs, report = evaluate(data, model, "test")
...
def load_model_decode(model_dir, data, name, gpu, seg=True):
    ...
    start_time = time.time()
    speed, acc, p, r, f, pred_results, gazs, report = evaluate(data, model, name)
    end_time = time.time()
...
if __name__ == '__main__':
    ...
    gaz_file = "../CNNNERmodel/data/tencent-ailab-embedding-zh-d200-v0.2.0-s.txt"
