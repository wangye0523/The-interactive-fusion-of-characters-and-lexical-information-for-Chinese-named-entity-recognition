...
def normalize_word(word):
    new_word = ""
    for char in word:
        if char.isdigit(): # 指示指定字符串中位于指定位置处的字符是否属于十进制数字类别
            new_word += '0'
        else:
            new_word += char
    return new_word
...
def read_instance_with_gaz(num_layer, input_file, gaz, word_alphabet, biword_alphabet, biword_count, char_alphabet,
    ...
    tokenizer = BertTokenizer.from_pretrained('../CNNNERmodel/data/bert-base-chinese', config=config, do_lower_case=True)

    in_lines = open(input_file, 'r', encoding="utf-8").readlines()
    # print("len_in_lines:",len(in_lines))
    instence_texts = []
    instence_Ids = []
    words = []
    biwords = []
        ...
        if len(line) > 2:
            ...
            if ((max_sent_length < 0) or (len(words) < max_sent_length)) and (len(words) > 0):
                ...
                for idx in range(w_length):
                    ...
                    # print("kkkkkkkkkkkkk",matched_list) #统计以idx开头的单词。
                ...
                instence_texts.append([words, biwords, chars, gazs, labels]) #words、labels
                instence_Ids.append(
                    [word_Ids, biword_Ids, char_Ids, gaz_Ids, label_Ids, gazs, gazs_count, gaz_char_Id, layergazmasks,
                     gazchar_masks, bert_text_ids]) # bert_text_ids、label_Ids

            elif len(words) >= max_sent_length:
                begin = 0
                while begin < len(words):
                    if begin+max_sent_length < len(words):
                        tempwords = words[begin:begin+max_sent_length]
                        tempbiwords = biwords[begin:begin+max_sent_length]
                        tempchars = chars[begin:begin+max_sent_length]
                        templabels = labels[begin:begin+max_sent_length]
                        tempword_Ids = word_Ids[begin:begin+max_sent_length]
                        tempbiword_Ids = biword_Ids[begin:begin+max_sent_length]
                        tempchar_Ids = char_Ids[begin:begin+max_sent_length]
                        templabel_Ids = label_Ids[begin:begin+max_sent_length]
                    else:
                        tempwords = words[begin:]
                        tempbiwords = biwords[begin:]
                        tempchars = chars[begin:]
                        templabels = labels[begin:]
                        tempword_Ids = word_Ids[begin:]
                        tempbiword_Ids = biword_Ids[begin:]
                        tempchar_Ids = char_Ids[begin:]
                        templabel_Ids = label_Ids[begin:]
                    gaz_Ids = []
                    layergazmasks = []
                    gazchar_masks = []
                    w_length = len(tempwords)
                    ...
                    for idx in range(w_length):
                        ...
                        matched_list = gaz.enumerateMatchList(tempwords[idx:])
                        # print("kkkkkkkkkkkkk",matched_list)
                        matched_length = [len(a) for a in matched_list]
                        matched_Id = [gaz_alphabet.get_index(entity) for entity in matched_list]
                    ...
                    texts = ['[CLS]'] + tempwords + ['[SEP]']
                    bert_text_ids = tokenizer.convert_tokens_to_ids(texts)

                    instence_texts.append([tempwords, tempbiwords, tempchars, gazs, templabels])
                    instence_Ids.append(
                        [tempword_Ids, tempbiword_Ids, tempchar_Ids, gaz_Ids, templabel_Ids, gazs, gazs_count, gaz_char_Id, layergazmasks,
                         gazchar_masks, bert_text_ids])
                    begin += max_sent_length
            ...
            char_Ids = []
            label_Ids = []
    # print("len_instence_texts:",len(instence_texts))
    # print("len_instence_Ids:",len(instence_Ids))
...
def build_pretrain_embedding(embedding_path, word_alphabet, embedd_dim=100, norm=True):
    ...
    if embedding_path != None:
        ...
        if embedding_path == "../CNNNERmodel/data/gigaword_chn.all.a2b.uni.ite50.vec":
